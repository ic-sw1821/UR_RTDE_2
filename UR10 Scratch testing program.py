...
while True:
    ...
    while keepRunning and scratches > 0:  # Waiting for 1 scratch to finish
        ...
        if not state.output_bit_registers0_to_31:
            ...
            print('Scratch no. ' + str(scratch_count) + ' finished. \n')
            keepRunning = False
            done = 0
            # data collection
            force = force.reshape((len(force) // 3), 3)
            position = position.reshape((len(position) // 3), 3)
            plotNoScratch = (len(force) - 1) // scratch_count
            timeData = np.linspace(0, plotNoScratch * 1 / FREQUENCY, plotNoScratch)
            timeData = timeData.reshape(len(timeData), 1)
            headerList = ["Time", "Force X", "Force Y", "Force Z", "Postion X", "Position Y", "Position Z"]
            # scratch data for each scratch
            scratchData = np.concatenate((timeData,
                                          force[:plotNoScratch],
                                          position[:plotNoScratch]),
                                         axis=1)
            scratchData = np.append([headerList], scratchData, axis=0)
            df = pd.DataFrame(scratchData)
            name = str(datetime.datetime.now()).replace("/", "-") + " Scratch" + str(scratch_count) + "file.csv"
